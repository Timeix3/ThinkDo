...
{
    Task<(IEnumerable<TaskItem> Items, int TotalCount)> GetAllWithProjectAsync(string userId, int offset = 0, int limit = 50);
    Task<TaskItem?> GetByIdWithProjectAsync(int id, string userId);
    ...
    Task<HashSet<int>> GetActiveSprintTaskIdsAsync(string userId);
}